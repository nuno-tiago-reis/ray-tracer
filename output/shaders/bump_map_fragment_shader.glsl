...
/* Input Attributes (Passed from the Bump-Map Vertex Shader) */
...
vec4 spotLight(int i, vec3 Normal, vec4 TextureColor) {
	...
	LightToVertex = convertToTangentSpace(LightToVertex, FragmentTangent, FragmentBitangent, FragmentNormal);
...
void main() {
	...
	/* Fragment Normal */
	vec3 Normal = normalize(texture2D(NormalTexture, FragmentTextureUV).xyz * 2.0 - 1.0);

	/* Fragment Texture */
	vec4 TextureColor = texture2D(DiffuseTexture, FragmentTextureUV);
